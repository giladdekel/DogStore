...
import {
  ...
  EMAIL_ORDER_SUCCESS,
  EMAIL_ORDER_FAIL,
  EMAIL_DELIVER_REQUEST,
  EMAIL_DELIVER_SUCCESS,
  EMAIL_DELIVER_FAIL,
...
export const emailDeliver = (orderId, userInfo) => async (
  dispatch,
  getState
) => {
  dispatch({
    type: EMAIL_DELIVER_REQUEST,
    payload: { orderId, userInfo },
  });
  try {
    const {
      userSignin: { userInfo },
    ...
    dispatch({
      type: EMAIL_DELIVER_SUCCESS,
      // payload: data.ticket,
    });
  } catch (error) {
    dispatch({
      type: EMAIL_DELIVER_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
...
export const emailOrder = (order, paymentResult) => async (
  dispatch,
  getState
) => {
  dispatch({
...
export const payOrder = (order, paymentResult) => async (
  ...
  try {
    ...
    dispatch(emailOrder(order, paymentResult));
...
export const deliverOrder = (orderId) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch(emailDeliver(orderId, userInfo));
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
