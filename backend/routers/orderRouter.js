...
// orderRouter.get("/activeOrder", async (req, res) => {
...
orderRouter.get(
  ...
  isAuth,
  expressAsyncHandler(async (req, res) => {
    const orders = await Order.find({ isActive: true, user: req.user._id });
    if (orders) {
      res.send(orders);
    } else {
    ...
    if (order) {
      ...
      mailgun()
        ...
        .send(
          {
            from:
              "DogBestFriends <postmaster@sandbox87b6fc885dba428e90fb48100dc137b8.mailgun.org>",
